...
if __name__ == "__main__":
    ...
    data_dev = [line.strip().split(", ") for line in lines_train]
    ...
    print("Time to create mapping is {0}s".format(time.time()-t1))
    ...
    t1 = time.time()
    error, _, predictions = knn_eval(bindata_dev, outputs_dev, bindata_train, outputs_train, k=41, o=1)
    print("Best error rate for k-NN is {0}% taking {1}s".format(error, time.time()-t1))
